...
class OntologyGraph:
    ...
    def __init__(self, graphs_dir: str | Path):
        ...
        self.reverse_adjacency: dict[str, dict[str, list[str]]] = {}
        self._load()
    ...
    def _load(self):
        ...
        for edge in data.get("edges", []):
            ...
            self.adjacency.setdefault(src, {}).setdefault(rel, []).append(tgt)
            self.reverse_adjacency.setdefault(tgt, {}).setdefault(rel, []).append(src)

        # Also load adjacency from precomputed section if available
        for node_id, adj in data.get("adjacency", {}).items():
            if node_id not in self.adjacency:
                self.adjacency[node_id] = adj
