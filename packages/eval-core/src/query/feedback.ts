...
export async function getAgentFailurePatterns(
  ...
  const rows = await db
    .select({
      ...
      failures: sql<number>`sum(case when ${agentInvocations.outcome} = 'failure' then 1 else 0 end)`.as('failures'),
      errorSummaries: sql<string>`group_concat(${agentInvocations.errorSummary}, '||')`.as('error_summaries'),
      lastFailureAt: sql<string | null>`max(case when ${agentInvocations.outcome} = 'failure' then ${agentInvocations.timestamp} end)`.as('last_failure_at'),
    })
...
export async function getSkillEffectiveness(
  ...
  const conditions = [
    sql`${agentInvocations.skillName} is not null`,
  ];
  if (since) {
    conditions.push(gte(agentInvocations.timestamp, since));
  }
  ...
  const rows = await db
    .select({
      ...
      total: sql<number>`count(*)`.as('total'),
      successes: sql<number>`sum(case when ${agentInvocations.outcome} = 'success' then 1 else 0 end)`.as('successes'),
      agentTypes: sql<string>`group_concat(distinct ${agentInvocations.agentType})`.as('agent_types'),
