...
**에이전트, 스킬, 규칙으로 Claude Code와 OpenAI Codex를 커스터마이징하는 가장 쉬운 방법.**
...
> **듀얼 프로바이더 지원**: Claude와 Codex를 모두 지원합니다. `omcustom init/update/list/doctor`에서 `--provider claude|codex`(또는 자동 감지)를 사용할 수 있습니다.
...
> 참고: 위 `opus/sonnet/haiku` 예시는 Claude 기준입니다. Codex-native 템플릿은 `reasoning/balanced/fast` 프로필 용어를 사용합니다.

실행 런타임이 적절한 모델을 선택하고, 독립적인 작업은 병렬 실행합니다 (최대 4개 동시):
...
정식 에이전트 ID 목록 (`templates/.claude/agents/*.md`):
...
정식 스킬 ID 목록 (`templates/.claude/skills/*/SKILL.md`):
