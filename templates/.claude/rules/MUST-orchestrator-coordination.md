...
| 위임서의 "요구사항 요약"을 오케스트레이터가 산문으로 재구성해 확정형으로 기재 (예: "…만 지정") | 이슈 제안 문장을 `gh issue view` 출력에서 복사해 전달; 오케스트레이터의 범위 요약에는 `[요약 — 원문 우선]` 표기 |
| 같은 주제 키워드 grep만으로 인접성 점검 완료 판정 | 키워드 grep + 삽입 지점 ±1 heading의 Anti-pattern 표 전수 대조를 병행 |
| 위임서에 오케스트레이터의 배선 판정("기존 bullet이 커버")을 확정형으로 기재 | 배선 대상 후보 파일·앵커만 제시하고 커버 여부는 서브에이전트가 해당 문장을 인용해 판정 |
