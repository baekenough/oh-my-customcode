...
> **v2.1.268 → 273 (도입 후 롤백, 네 번째)**: (268) `env -C`, `eval` 등 권한 검사기가 분석할 수 없는 명령이 같은 줄에 있으면 Read/Edit deny 규칙이 적용되지 않던 결함이 수정되었으나, **273이 이를 롤백**했습니다 — `time -p make build` 같은 명령이 거부되는 대신 다시 프롬프트됩니다. 이 파일에서 네 번째로 관측되는 "도입 후 롤백" 사례입니다(v2.1.232/233, v2.1.259→260에 이어). 또한 (268) macOS의 `/etc`, `/tmp`, `/var`, Linux의 `/bin`처럼 symlink된 디렉토리에 대한 deny/ask 규칙이 경로를 실제 위치(real location)로 표기했을 때 적용되지 않던 결함, 그리고 Bash 명령이 symlink 경로 표기로 작성된 deny 규칙을 무시하던 결함이 수정되었습니다(경로 표기 우회 계열 — 위 v2.1.233 `\??\`, v2.1.251 (c)(d)와 동류). 이 저장소 관련성: 이 저장소의 훅은 `/tmp` 아래에 PPID 스코프 마커를 씁니다(R010 carve-out) — macOS에서 `/tmp`는 symlink이므로, 268 이전에는 `/tmp/...`로 적은 deny 규칙이 `/private/tmp/...`를 커버하지 못했습니다.

> **v2.1.269/270+**: (269) `!`로 시작하는 deny/ask permission rule이 그 규칙을 작성한 settings 소스 범위를 넘어 적용되던 결함이 수정되어, 이제 **자기 소스 내에서만** 적용되고 단독 `!` 부정은 무시됩니다. `Edit()` deny 규칙과 write-path 검사가 Bash `tee`가 쓰는 파일에는 적용되지 않던 결함, `Bash(tee:*)` allow 규칙이 작업 디렉토리 밖 목적지까지 커버하던 결함도 수정되었습니다. `--output-format stream-json`의 `permission_denials`가 경로-스코프 deny 규칙에 막힌 Read/Edit/Write 호출을 누락하던 결함도 수정되었습니다(`-p` 실행의 거부 텔레메트리가 과소 계상되고 있었습니다). (269) **회귀**: 세션이 한동안 실행된 뒤 Bash의 read-only git 명령이 예상치 못하게 권한을 요청하던 결함이 270에서 수정되었습니다 — 269 단독 세션에서 갑자기 git 권한 프롬프트가 뜬 것은 settings 변경이 아니라 플랫폼 회귀였습니다.

> **v2.1.271+**: Bash 권한 검사 하드닝 5건입니다 — (a) 검사기가 인식하지 못하는 옵션 뒤에 `fmt`, `column` 등의 명령이 읽는 파일이 누락되던 결함, (b) 와일드카드가 패턴이나 옵션 값 안에 있을 때(예: `grep -v dir/* file`) 그 와일드카드가 확장하는 파일들이 누락되던 결함, (c) 셸 변수 선언 플래그가 실행 중인 명령을 잘못 나타낼 수 있던 결함, (d) 디렉토리 변경 2회, 서브셸, 또는 `cd`+`git` 체인을 가진 명령이 bypass·auto 모드의 `permissions.blockReadsOutsideWorkingDirectories`에서 프롬프트를 건너뛰던 결함이 각각 수정되었습니다. (e) 신규 기능으로 auto mode의 샌드박스에서 Bash/PowerShell/Monitor에 대해 **명령 단위 `allowed_domains`**가 도입되어, 명령이 필요로 하는 호스트가 그 명령과 함께 검토되고 그 명령에만 열립니다. 위 v2.1.251/257의 5건 묶음 노트와 같은 계열입니다 — 한 릴리즈에 다수 우회가 몰려 있으므로, 권한 검사는 계속 defense-in-depth로만 취급합니다.

> **v2.1.273/274+**: (273) 권한 검사기가 완전히 분석할 수 없는 Bash 명령이 `permissions.blockReadsOutsideWorkingDirectories`에서 프롬프트를 건너뛰던 결함, bypass 모드에서 서브셸이 위험한 `rm`을 숨길 수 있던 결함이 수정되었습니다(R001 파괴적 명령 교차 참조). macOS에서 드래그 앤 드롭한 스크린샷이나 시스템이 두 번째 경로로 보고하는 파일에 대해 Read가 "symlink resolution changed after permission was checked"로 거부하던 결함도 수정되었습니다(위 v2.1.251 (c) TOCTOU 가드의 오탐이었지, 우회가 아니었습니다). (274) 특정 특수 셸 변수를 반복문에서 쓰거나 대입하는 Bash 명령이 이제 권한을 요청합니다(위 v2.1.251 (a) `OPTIND=`/`RANDOM=` 노트의 확장). 워크트리 격리 세션이 특정 중첩 셸 확장을 포함한 Bash 명령을 이제 **거부**합니다 — 위 v2.1.257 (e)의 완화와 반대 방향으로 움직였다는 점에 유의합니다. R009 워크트리 위임에서 복합 확장이 다시 거부될 수 있으니, 격리 결함으로 진단하지 말고 명령을 단순화합니다.

> **v2.1.275+**: (275) `Fixed "/update-config" writing "Write(path)" permission rules, which file permission checks don't match, instead of "Edit(path)" rules` — 파일 쓰기 rule은 `Edit(path)`, 읽기 rule은 `Read(path)` matcher로 작성한다는 권고(v2.1.210, 현재 은퇴)와 정합하는 수정입니다; 구버전 `/update-config`가 만든 `Write(path)` allow 규칙은 실제 파일 권한 검사에 매칭되지 않아 무효였으므로, settings의 allow 규칙을 감사할 때 `Write(...)` 형태를 발견하면 `Edit(...)`로 교정합니다. (275) `Fixed sandboxed Bash commands being unable to write to project directories named "hooks/" or "config/"` — 이 저장소의 `.claude/hooks/`가 정확히 이 이름 패턴에 해당하며, 구버전 샌드박스에서는 훅 스크립트 편집 Bash 명령이 조용히 실패할 수 있었으므로, 275 이전 세션에서 관측된 훅 파일 미반영을 권한 거부만으로 진단하지 않습니다(R020 Read-Before-Characterize).
