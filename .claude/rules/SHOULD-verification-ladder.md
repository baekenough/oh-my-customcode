...
검증 ladder: **결정론적 → cheap LLM → expensive LLM → human**. 저렴한 tier 통과 후 다음으로 진행.

<!-- DETAIL: Core Rule, original wording
...
Tier 3 검증은 background agent로 실행될 수 있어 반환이 착수 신호일 뿐이다 — 통과 판정 전 실제 완료를 실측한다(R020).

<!-- DETAIL: v2.1.232 background-agent note, original wording
...
결정론적 단계가 잡을 수 있는 문제는 LLM에 보내지 않는다(예: schema 오류는 Tier 1이 차단, LLM 미전달).

<!-- DETAIL: Shift-left 원칙 examples and R013 cross-ref, original wording
...
T1: hooks, `mgr-sauron`, linters. T2: `dev-review`, `action-validator`, `pre-generation-arch-check`. T3: `deep-verify`, `adversarial-review`, `multi-model-verification`, `evaluator-optimizer`, `worker-reviewer-pipeline`. T4: maintainer review.

<!-- DETAIL: 기존 자산 매핑 table, original wording
| Tier | 자산 | 역할 |
|------|------|------|
...
R021 (MUST-enforcement-policy)과 R023은 **직교**한다(서로 다른 차원):

<!-- DETAIL: R021과의 관계 intro, original wording
...
같은 도구가 두 규칙에 속할 수 있다(`mgr-sauron`=R021 Advisory+R023 Tier1). R021=멈출지, R023=순서.

<!-- DETAIL: R021과의 관계 예시 목록, original wording
...
Fable 5는 지시 장문화가 품질을 저하시킬 수 있다 — 신규 규칙/스킬은 간결성 우선.

<!-- DETAIL: Fable 5 Over-Prescription Advisory, original wording
...
안전-신호 룰(인터럽트·halt·emergency-stop 등) 추가/수정 시, 작성 단계(Tier 1)에서 다음을 사전 점검한다:

<!-- DETAIL: Safety-Signal intro sentence, original wording
...
불확실하면 carve-out을 먼저 명시하거나 Tier 3 적대적 검증 통과 후 release한다(R023 shift-left를 룰 작성 자체에 적용).

<!-- DETAIL: Safety-Signal trailing paragraph, original wording
...
staleness 검증은 model ID/placeholder뿐 아니라 **폐기된 플랫폼 기능·절차 참조**도 스캔한다(R023 shift-left).

<!-- DETAIL: Deprecated-Platform-Feature Staleness Check paragraph, original wording
...
|--------------|----------|
| staleness 스캔을 model ID/placeholder로만 한정 | 폐기된 플랫폼 기능/절차 참조도 grep 스캔 |

<!-- DETAIL: Deprecated-Platform-Feature anti-pattern table row, original wording
...
문자열 조립 위임 시 **순수 조립부를 Tier-1 로컬 결정론 검증으로 분리**한다 — 문법 검증만으로는 들여쓰기·형식 결함이 안 드러난다.

<!-- DETAIL: Sample-Value Assembly main paragraph, original wording
...
| Anti-pattern | Required |
|--------------|----------|
| "스모크 테스트=API 호출"로 협소 인식 → 순수 조립 검증을 실서버 Tier로 미룸 | 위임 첫 프롬프트부터 "샘플 값 조립 후 grep 검증" 필수 명시 |
| `textwrap.dedent(f"""...{multiline_var}...""")` 를 신뢰 | 0-indent 멀티라인 변수가 dedent를 무력화 → 리터럴 들여쓰기 잔존. concatenation 사용 |

<!-- DETAIL: Sample-Value anti-pattern table row 1, original wording
...
<!-- DETAIL: Sample-Value anti-pattern table row 2, original wording
...
**키·해시 설계 위임서**: 완료 조건에 저장소 대표 편집 픽스처로 충돌 선확인을 포함한다(Tier 1 검증이 Tier 3 비용 절감).

<!-- DETAIL: 키·해시 설계 위임서, original wording
...
**리서치 위임의 결론 수치는 표에서 재계산해 병기**: 결론 수치는 표에서 jq/awk로 재계산 후 병기하도록 완료 조건에 명시한다.

<!-- DETAIL: 리서치 위임 결론 수치 paragraph, original wording
**리서치 위임의 결론 수치는 표에서 재계산해 병기 (Origin: #1707 #4 — v1.1.75)**: 리서치·계측 위임의 완료 조건에는 결론 문장에 등장하는 모든 수치를 산출물 자체의 표에서 jq/awk 등으로 재계산하고, 그 값을 결론 옆에 병기하도록 명시합니다. 원인은 "리서치 에이전트가 표를 먼저 쓰고 결론을 별도 산술로 작성(표에서 재계산하지 않음)"이며, 결론 문장의 합계(697)가 표(354)와 불일치한 산술 모순 사례가 실증입니다.
-->

| Anti-pattern | Required |
|--------------|----------|
| 결론 문장의 수치를 표와 별도로 산술해 기재 | 결론 수치를 표에서 jq/awk로 재계산하고 결론 옆에 병기하도록 완료 조건에 명시 |

배선: auto-dev.yaml deep-plan 스텝 (4사본), 「Delegated Verification Floor」와 동일 방식.

<!-- DETAIL: 배선 note, original wording
배선: auto-dev.yaml deep-plan 스텝 description (4사본) — 「Delegated Verification Floor」 상한선 조항과 같은 방식입니다.
-->

**오케스트레이터가 전달하는 보고 수치도 재계산 의무**: 다음 위임서로 옮기기 전 재계산한다(`git diff --stat`/`grep -c` 등).

<!-- DETAIL: 오케스트레이터 재계산 의무 paragraph, original wording
...
배선: auto-dev.yaml implement 스텝 (4사본).

<!-- DETAIL: 배선 note 2, original wording
배선: auto-dev.yaml implement 스텝 표준 제약 블록 (4사본).
-->

## Delegated Verification Floor — CI 잡 목록에서 도출 (Origin: #1574)

**하한선은 CI가 실제로 돌리는 잡 전체**다 — 워크플로 YAML 잡의 로컬 대응 명령을 완료 조건에 포함한다.

<!-- DETAIL: Delegated Verification Floor lower-bound paragraph, original wording
...
**상한선**: 사전 실측 항목은 위임서에 **재실행 금지 목록**으로 열거한다(재실행 시 턴 예산 소진, R020).

<!-- DETAIL: 상한선 paragraph, original wording
...
조건부 출력 검증 위임 시, 완료 기준은 **양성/음성 입력을 짝으로** 지정한다 — 단일 프록시는 검증이 아니다.

<!-- DETAIL: Conditional-Output Verification main paragraph, original wording
...
**래퍼·재사용 스크립트 위임의 음성 픽스처**: 원본 제외 계열을 전부 열거하고 계열별 적용/미적용 판정 + 음성 픽스처를 요구한다.

<!-- DETAIL: 래퍼·재사용 스크립트 위임 paragraph, original wording
...
**훅 옵션 검증은 합성 픽스처 + 실 트랜스크립트 1건**: 훅 옵션 변경 위임 시, 실 트랜스크립트 1건 계수 실측도 완료 조건에 포함한다.

<!-- DETAIL: 훅 옵션 검증 paragraph, original wording
...
검출 가드 위임 시 positive-match와 negative-context를 구분한다(누락 시 자기모순 BLOCK).

<!-- DETAIL: Detection Guard Delegation Standard paragraph, original wording
...
| Anti-pattern | Required |
|--------------|----------|
| bare 패턴만 grep → "폐기됨/불필요" 설명 문구까지 오탐 | positive-match(MUST 인접) vs negative-context(deprecated 설명) 구분 기준 명시 |

<!-- DETAIL: Detection Guard anti-pattern table row, original wording
...
Workflow prompts MUST be assembled **before** the `agent()` call — post-call concat is a footgun.

<!-- DETAIL: Prompt Completion Before Call, original wording
...
| Check | Why |
|-------|-----|
| No unresolved placeholders remain in any agent prompt string | Unfilled placeholder reaches agent verbatim → garbled task |
| Assemble-before-call (see above) | Post-call concatenation silently drops content |
| Script parses — balanced braces/quotes, valid JS | Syntax error aborts entire run after partial work |
| 셸 변수 `${...}`가 `\${...}`로 이스케이프됐는지 grep 확인 | 미이스케이프 시 JS가 표현식으로 평가 → 런타임 `ReferenceError`; `node --check`는 못 잡음 |
| `args`가 문자열/객체 방어를 거치는지 확인 | 문자열로 인코딩되면 `args.<field>` undefined로 즉시 실패 |

<!-- DETAIL: Workflow Script Sanity Check table, original wording
| Check | Why |
|-------|-----|
...
Workflow 도구 견고성·안전성 CC 버전 노트(223~274)는 DETAIL 참조.

<!-- DETAIL: Workflow Script Sanity Check version notes (v2.1.223~274), original wording
...
Common Violation 사례(#1271/#1438/#1512)는 DETAIL 참조 — 전부 Tier-1 sanity check로 사전 포착 가능했다.

<!-- DETAIL: Common Violation examples, original wording
...
Cross-cutting facts unverifiable from primary source MUST be supplied as explicit ground-truth.

<!-- DETAIL: Verifier Ground-Truth paragraph, original wording
