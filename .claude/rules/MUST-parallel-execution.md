...
> **v2.1.269/271+**: (269) `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`가 신설되어 Workflow 스크립트가 한 번에 실행하는 agent 수를 상한할 수 있습니다. (271) dynamic workflow는 usage limit에 도달하면 이제 agent를 실패시키는 대신 **일시정지 후 한도 리셋 시 재개**합니다. 이 규칙의 "Max instances 5 concurrent (soft default 4)" cap과의 관계: 플랫폼 env var는 **Workflow-tool fan-out에만** 적용되므로, R009의 cap은 Agent-tool 스폰을 규율하고 이 env var는 Workflow 스크립트 내부의 `parallel()`/`pipeline()`을 규율합니다 — 둘 다 일관되게 설정합니다. 또한 (269) 응답이 output-token 절단에 걸린 뒤 prompt cache가 무효화되어 이어지는 턴이 전체 prefix 비용을 다시 지불하던 결함이 수정되었습니다 — 위 v2.1.229 stagger 노트와 결합하면, 병렬 형제는 이제 절단된 응답 이후에도 캐시 재사용을 유지합니다.
