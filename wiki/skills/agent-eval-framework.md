...
Six categories map to oh-my-customcode tools: `file_operations` (Write/Edit), `retrieval` (Glob/Grep/Read), `tool_use` (Agent/Bash), `memory` (native auto-memory), `conversation` (routing skills), `summarization` (result-aggregation). Aim for ≥ 3 tasks per category.
