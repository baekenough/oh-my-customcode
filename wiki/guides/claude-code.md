...
> Updated: 2026-05-29
> Source: Claude Code release notes (#967, #968, #969, #1126 auto-detected by claude-native skill, #1137, #1158, #1187, #1191, #1242, #1243, #1244, #1245, #1241)

## Compatibility Baseline

oh-my-customcode v0.107.0 targets Claude Code **v2.1.156+** (updated 2026-05-29). All v2.1.117-156 additions are backward-compatible unless otherwise noted — no config changes required.
...
| v2.1.146 | docs-only. `/simplify`→`/code-review` 리네임 + effort level, `CLAUDE_CODE_SUBAGENT_MODEL` 전파 fix. | None |
| v2.1.147 | `Workflow` 도구(`CLAUDE_CODE_WORKFLOWS=1`) 추가 — /pipeline과 개념 중첩, 통합 검토 후보. plugin agent 복수 `Agent()` 타입 fix. | P3 follow-up |
| v2.1.148 | Bash 도구 exit 127 regression(v2.1.147 도입) 수정 — v2.1.147 사용 시 즉시 업그레이드. | None |
| v2.1.149 | `/usage` per-category breakdown, Bash `find` macOS 크래시 fix — 업그레이드 권장. | P3 follow-up |
| v2.1.150 | 내부 인프라 개선만, 사용자 대면 변경 없음. | None |
| v2.1.152 | Skill `disallowed-tools` frontmatter (R002/R006), `/reload-skills`, `SessionStart reloadSkills` — no harness change needed. | None |
...
| v2.1.154 | Opus 4.8 + `opus48` alias (R006), Dynamic Workflows GA (R009/R018), lean system prompt default, `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE` deprecated 06/01. | P2 action |
| v2.1.156 | Opus 4.8 thinking-block API-error fix — v2.1.154 사용 시 즉시 업그레이드. | None |
| #1241 | Agent tool malformed-parsing on long/special-char prompts — workarounds documented in Known Platform Issues. | See below |
...
- `/simplify` → `/code-review` 리네임 (+ effort level, `--comment`) — 내부 `dev-review` 스킬과 충돌 없음
- Auto mode `AskUserQuestion` 억제 해제 — R015 ambiguity-gate 정상 작동 개선
- MCP `resources/list` 페이지네이션 수정 — `ListMcpResourcesTool` (R011/R019) 안정성 향상
- `CLAUDE_CODE_SUBAGENT_MODEL` 자식 프로세스 전파 수정 — R010+R018 Agent Teams 멤버 모델 일관성 개선

**Action items**: 코드 변경 없음 (docs-only). `CLAUDE_CODE_SUBAGENT_MODEL` 전파 동작 검증 권장.
...
- `Workflow` 도구 추가 (`CLAUDE_CODE_WORKFLOWS=1` 활성화) — oh-my-customcode `/pipeline` 스킬과 개념 중첩, 중장기 통합 후보
- `/simplify` → `/code-review` 개명 확정 (v2.1.146 연속) — `agent-triggers.yaml` 점검 권장
- REPL/Workflow 샌드박스 강화 — R001 보안 강화
- plugin agent 복수 `Agent()` 타입 fix — frontmatter `tools:` 복수 선언 이제 정상 동작
- hook `if` 조건 매칭 버그 수정 — stage-blocker hook (R001) 신뢰성 향상

**Action items**: agent-triggers.yaml `skill-simplify` 엔트리 명명 혼동 방지 점검. Workflow 도구와 /pipeline 통합 연구 후보 (P3).
...
- **Bash 도구 exit code 127 regression 수정** (v2.1.147 도입) — v2.1.147 사용 중이라면 즉시 업그레이드 권장

**Action items**: v2.1.147 사용 환경은 즉시 v2.1.148로 업그레이드.
...
- `/usage` per-category breakdown (skills/subagents/plugins/MCP) — R013 ecomode 토큰 추적 개선
- **Bash `find` macOS 크래시 버그 수정** — 대형 디렉토리 탐색 호스트 안정성 향상 (업그레이드 권장)
- GFM 체크박스 렌더링 — 에이전트 작업 목록 가독성 향상
- git worktree sandbox write allowlist 수정 — R001 보안 강화

**Action items**: macOS 환경에서는 Bash `find` 크래시 수정을 위해 업그레이드 권장.
...
내부 인프라 개선만 포함. 사용자 대면 변경 없음. 코드 및 문서 변경 불필요.
...
> **Note**: v2.1.151은 공개 릴리즈가 없었습니다.

- **Skill `disallowed-tools` frontmatter** (R002/R006) — 스킬 실행 중 특정 도구를 제거하는 세밀한 tool scope 제어. 조사/분석 전용 스킬에서 `disallowed-tools: [Bash, Write]` 패턴 활용 가능
- `/reload-skills` 커맨드 + `SessionStart reloadSkills: true` — 세션 재시작 없이 스킬 디렉토리 재스캔 (R006)
- `SessionStart` 훅 세션 제목 설정 (`hookSpecificOutput.sessionTitle`) — R012 HUD 강화 후보
- `MessageDisplay` 훅 이벤트 추가 — 어시스턴트 메시지 표시 시점 변환 가능 (R001 훅 변경 승인 필요)
- `--fallback-model` — R006 `model:` 프론트매터 보완 수단 (CI 환경 opt-in)

**Action items**: `disallowed-tools` frontmatter를 read-only 스킬에 선택적 적용 검토. `SessionStart sessionTitle`을 R012 HUD 창 제목 통합에 활용 검토 (P3).
...
- **Statusline scripts `COLUMNS`/`LINES` 환경 변수 수신** (R012) — `.claude/statusline.sh` 반응형 개선 가능 (P3)
- `skipLfs` 플러그인 마켓플레이스 옵션 — LFS 에셋 포함 플러그인 설치 속도 개선 (opt-in)
- `claude agents` 자동완성 — 네이티브 슬래시 커맨드 및 번들 스킬 포함
- `/model` 기본값 저장 방식 변경 (`s` 키: 현재 세션만 변경, 기본은 새 세션 기본값 저장)
- MCP 서버 메모리 과다 사용 수정 — R011 장시간 세션 안정성 향상
- `subagent_type: 'claude'` gitignored 경로 무음 폐기 수정 — R010 아티팩트 채널 신뢰성 향상

**Action items**: `.claude/statusline.sh`에 `COLUMNS` 기반 반응형 출력 적용 검토 (R012, P3). keybindings.json에서 `modelPicker:setAsDefault` 커스터마이즈 시 `modelPicker:thisSessionOnly`로 rename 필요.
...
> **Note**: v2.1.155는 공개 릴리즈가 없었습니다.

### Opus 4.8 도입 (R006 model aliases)

Opus 4.8이 출시되었습니다. 기본 effort는 `high`이며, `/effort xhigh`로 최고 추론 강도를 활성화합니다.

**oh-my-customcode 연관**: R006 model aliases 표에 `opus48` alias 추가 필요 (`claude-opus-4-8` 매핑). 현재 `opus` alias는 `claude-opus-4-6`을 가리킵니다. **`CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE` 환경 변수는 2026-06-01 폐기 예정** — 해당 변수를 사용 중이라면 `/model claude-opus-4-6[1m]` + `/fast on` 패턴으로 전환 필요.

### Dynamic Workflows GA (R009/R018)

`Workflow` 도구가 일반 공개되었습니다. `/workflows`로 실행 중인 워크플로우를 확인할 수 있습니다. oh-my-customcode `/pipeline` 스킬과의 통합 가능성이 높아졌습니다 (P2 중장기 후보).

### Lean system prompt 기본값 변경

Haiku, Sonnet, Opus 4.7 이하를 제외한 모든 모델(Opus 4.8 포함)의 기본값이 lean system prompt로 변경됩니다. R013 ecomode context budget에 긍정적 영향.

### 기타

- `AskUserQuestion` 자제 개선 — R015 ambiguity-gate 불필요한 확인 요청 감소
- `CLAUDE_CODE_SESSION_ID` Stdio MCP 서브프로세스 전달 — R011 메모리 MCP 세션 스코핑 활용 가능
- `rm -rf $HOME` trailing slash 위험 경로 차단 수정 — R001 보안 강화

### oh-my-customcode 연관 평가

| 변경 | 영향 | 조치 |
|------|------|------|
| Opus 4.8 + `opus48` alias | R006 model aliases 업데이트 필요 | P2 이슈 등록 권장 |
| `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE` 폐기 06/01 | 해당 변수 사용 환경 전환 필요 | 06/01 이전 조치 |
| Dynamic Workflows GA | R009/R018 + /pipeline 통합 후보 | P2 follow-up |
| Lean system prompt 기본값 | Opus 4.8 에이전트 토큰 감소 | None (자동) |
| `rm -rf $HOME` trailing slash fix | R001 보안 강화 | None |

**Action items**: `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE` 사용 환경은 2026-06-01 이전에 전환 필요. R006 `opus48` alias 추가는 별도 이슈로 추적.
...
**oh-my-customcode 연관**: v2.1.154로 Opus 4.8을 도입한 경우 v2.1.156으로 업그레이드가 강력히 권장됩니다. 직접 harness 변경 불필요.

**Action items**: v2.1.154 사용 환경은 즉시 v2.1.156으로 업그레이드.
...
이 패턴은 `docs/superpowers/plans/` **직접 자식** `.md` 파일만 추적합니다. git 시맨틱상 부모 디렉토리가 이미 제외(`*`)되면, 자식 디렉토리 내 파일의 `!` 부정 패턴이 효력을 발휘하지 않습니다.

**현재 영향**: 없음. `release-plan` 스킬은 플랫 경로만 생성합니다.
...
**증상**: Agent (subagent dispatch) 도구가 위임 프롬프트가 매우 길거나 특수 문자(백틱, 연속 콜론, 셸 변수 구문)가 많을 때 `malformed`로 보고하며 간헐적으로 파싱 실패가 발생합니다. 이는 Claude Code 플랫폼 레벨의 직렬화 이슈이며 oh-my-customcode 결함이 아닙니다.

**Workarounds** (기존 규칙과 정합):
- 위임 프롬프트가 ~5000 토큰을 초과하거나 3개 이상 무관한 도메인을 포함하면 도메인별 병렬 에이전트로 분해 (R009 giant-prompt anti-pattern)
- 위임 프롬프트에서 무거운 리터럴 블록보다 평문 산문 선호; 긴 펜스 코드 블록(백틱)과 연속 `::` 시퀀스를 평문으로 대체 가능한 경우 대체
- 대용량 페이로드가 불가피하면 프롬프트에 인라인 삽입 대신 아티팩트 파일 경로를 통해 핸드오프 (R006 Artifact Channel Protocol)
