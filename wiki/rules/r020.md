...
## Verification-Delegation Non-Termination (#1443)
...
mid-step 종료가 발생하면 판정을 그대로 신뢰하지 말고(Core Rule) SendMessage로 해당 에이전트를 resume하여 최종 판정을 받는다.

| Anti-pattern | Required |
|--------------|----------|
| 검증/판정 위임 프롬프트에 종료 조건 미명시 → 판정 없이 mid-step 종료 | 위임 프롬프트에 "판정 없이 turn 종료 금지" 명시 + mid-step 종료 시 SendMessage resume |

**Common Violation (#1443):** v1.1.3 [[r017]] 검증에서 mgr-sauron이 source-hash 대조 중 판정 없이 종료 → resume 후 PASS. v1.1.4에서 "판정 반드시 출력" 명시로 1회 완료(대조 실증).
