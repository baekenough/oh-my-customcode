...
| R018 | MUST (조건부) | Agent Teams | CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1 AND `TeamCreate` 도구 실재 시 필수; 아니면 dormant |
...
| 카테고리 | 수량 | 에이전트 |
|----------|------|----------|
| SW Engineer / 언어 | 6 | lang-golang-expert, lang-python-expert, lang-rust-expert, lang-kotlin-expert, lang-typescript-expert, lang-java-expert |
...
`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` **그리고** `TeamCreate` 도구가 도구 목록에 실재할 때만 활성화됩니다 — env 변수 단독은 활성 근거가 아니며, `SendMessage` 존재도 증거가 아닙니다. CC v2.1.233+ 기준 `TeamCreate`는 도구 목록에 미등록(실측)이므로 현행 런타임에서 R018은 dormant이며 R009/R010이 지배합니다. 활성화 상태에서 기준을 충족하면 사용이 **의무**입니다.
...
triage-complete 이슈를 수집하고 우선순위와 크기에 따라 릴리즈 유닛으로 그룹화합니다. 구현 순서와 에이전트 추천이 포함된 구조화된 릴리즈 계획 문서를 생성합니다.
...
`packages/memory-mcp-server/`는 v0.123.0에서 추가된 내장 메모리 MCP 서버입니다. 4개의 MCP 도구(save_memory, recall_memory, list_memories, delete_memory)를 통해 여러 메모리 백엔드를 통합 인터페이스로 노출합니다.

```
packages/memory-mcp-server/
  src/tools/    — 4개 MCP 도구 구현
  src/adapters/ — 네이티브 메모리 어댑터
  src/service/  — 집계 + 중복 제거 계층
```

### 9.3a Skill Profiles

v0.123.0에서 추가된 스킬 프로필 시스템입니다. `/profile` 커맨드로 세션 시작 시 사전 정의된 스킬 세트를 로드합니다. 4개의 기본 프로필이 제공됩니다: `default`, `research`, `engineering`, `review`.
...
| v0.121.0 | 메모리 집계 + 중복 제거 계층; 다중 어댑터 통합 |
| v0.120.0 | 메모리 통합 어댑터 (네이티브 메모리 어댑터 계층); MemoryRecord TypeScript 인터페이스 |
| v0.118.0 | init 자동 설정; context:fork 스킬 분할 (대형 SKILL.md → 핵심 + phases.md); 토큰 관측성 개선 |
| v0.116.2 | R010 Universal /tmp Script Bypass; .claude/ 경로 Bash/Write/Edit 모두 /tmp/*.sh 경유 필수 (민감 경로 처리) |
| v0.115.0 | LangChain harness/middleware 통합 가이드; 에이전트 평가 프레임워크 확장 |
